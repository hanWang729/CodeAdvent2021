...
bool is_action_helpful (const State& state, const Blueprint& blueprint, const Action& action) {
...
bool is_action_possible (const State& state, const Blueprint& blueprint, const Action& action) {
  if (action == Action::ORE) {
...
int main(int argc, char * argv[]) {
  ...
  for (auto& blueprint : blueprints) {
    ...
    for (const auto action : actions) {
      dfs(init_state, blueprint, max_geodes, actions, action);
    }
    blueprint.max_geodes = max_geodes;
    ans += blueprint.id * blueprint.max_geodes;
  }
  std::cout << ans << '\n';
  return 0;
}